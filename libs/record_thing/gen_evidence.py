...
from vertexai.vision_models import ImageGenerationModel, Image # Use stable Vision Models
...
class EvidenceImageGenerator:
    def __init__(self, 
        ...
        self.rate_limit_delay = 3  # seconds between API calls (adjust per provider if needed)
        self.max_retries = 3
        self.image_size = image_size 
        self.image_quality = image_quality if image_quality in ["standard", "hd"] else "standard" # OpenAI/Azure DALL-E 3
        self.image_model_name = image_model # Store the requested model/deployment name
        self.model_params = model_params or {} # Initialize model_params as empty dict if None
        self.iconic_size = iconic_size  # Size for thumbnail images stored in the database

        # --- Provider specific initialization ---
        if self.provider == "openai":
            self.api_key = api_key or os.getenv("OPENAI_API_KEY")
            if not self.api_key:
                raise ValueError("OpenAI API key is required (or set OPENAI_API_KEY environment variable)")
            ...
            if not self.project_id:
                raise ValueError("Google Cloud project_id is required (or set GOOGLE_CLOUD_PROJECT environment variable)")
            
            logger.info(f"Initializing Google Cloud Vertex AI: Project={self.project_id}, Location={self.location}")
            try:
                aiplatform.init(project=self.project_id, location=self.location)
                self.image_model_name = self.image_model_name or "imagegeneration@006"
                self.google_model = ImageGenerationModel.from_pretrained(self.image_model_name)
                logger.info(f"Using Google Vertex AI provider with model: {self.image_model_name}")
            ...
            self.image_model_name = self.image_model_name or os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME") 
            if not all([self.api_key, self.azure_endpoint, self.api_version, self.image_model_name]):
                raise ValueError("Azure provider requires api_key, azure_endpoint, api_version, and image_model (deployment name). Set them or use environment variables.")
            self.azure_client = AzureOpenAI(
                api_key=self.api_key, azure_endpoint=self.azure_endpoint, api_version=self.api_version
            )
            logger.info(f"Using Azure OpenAI provider with deployment: {self.image_model_name} at {self.azure_endpoint}")

        elif self.provider == "together":
            self.api_key = api_key or os.getenv("TOGETHER_API_KEY")
            if not self.api_key:
                raise ValueError("Together AI API key is required (or set TOGETHER_API_KEY environment variable)")
            ...
            self.image_model_name = self.image_model_name or "stabilityai/stable-diffusion-v1-5"
            logger.info(f"Using Together AI provider with model: {self.image_model_name}")
            # Parse image size for height/width
            try:
                self.width, self.height = map(int, self.image_size.split('x'))
            except ValueError:
                logger.warning(f"Invalid image_size format '{self.image_size}' for Together AI. Defaulting to 1024x1024.")
                self.width, self.height = 1024, 1024

        else:
            raise ValueError(f"Unsupported provider: {provider}. Choose 'openai', 'google', 'azure', or 'together'.")
    ...
    def get_missing_evidence(self, limit: Optional[int] = None) -> List[Dict]:
        ...
        for row in all_evidence:
            ...
            if row[10] and row[10] != 'null':
                try:
                    ...
                    image_url = evidence_data.get('image_url') or evidence_data.get('url')
            ...
            if not image_url:
                try:
                    ...
                    logger.warning(f"Error querying image_assets table for {evidence_id}: {e}")
            
            # 3. If we found a URL, try to download the image
            if image_url:
                logger.info(f"Found URL for missing evidence {evidence_id}: {image_url[:100]}...")
            ...
            logger.debug(f"Missing file found: {file_path} for evidence ID {evidence_id}")
        ...
        logger.info(f"Found {len(missing_evidence_list)} evidence records with missing image files (after recovery attempts).")
    ...
    def sanitize_prompt(self, text: str) -> str:
        ...
        if self.provider == "openai" or self.provider == "azure":
            ...
            max_length = 2000  # Stable Diffusion models generally have a bit more capacity
        elif self.provider == "google":
            max_length = 1500  # Imagen is more restrictive
            
        if len(sanitized) > max_length:
            logger.debug(f"Truncating prompt from {len(sanitized)} to {max_length} characters")
            sanitized = sanitized[:max_length]
            # Ensure we don't cut in the middle of a sentence
            last_period = sanitized.rfind('.')
            if last_period > max_length * 0.8:  # If there's a period in the last 20% of text
    ...
    def generate_prompt(self, evidence: Dict) -> str:
        ...
        try:
            logger.debug(f"Generating prompt for evidence: {json.dumps(evidence, indent=2)}")
            ...
            is_document = (
                document_type != "" or 
                evidence_type.lower() in ["document", "receipt", "invoice", "certificate", "letter", "id", "identification"] or
                (evidence.get('category', "").lower() in ["document", "documents", "paperwork", "certificate", "identification"])
            )
            ...
            item_desc_parts = [evidence.get('brand'), evidence.get('thing_title', 'a personal item')]
            if evidence.get('thing_description'):
                item_desc_parts.append(f"described as '{evidence.get('thing_description')}'")
            ...
            if not is_document:
                context_variants.append("showing the back side or alternate angle, revealing additional details")
    ...
    def _create_iconic_image(self, image_data: bytes) -> bytes:
        ...
        try:
            ...
            logger.debug(f"Created iconic image: {len(iconic_png)} bytes, size {img.size}")
            return iconic_png
        except Exception as e:
    ...
    def _save_to_image_assets(self, path: str, image_data: bytes, image_url: Optional[str] = None) -> bool:
    ...
    def generate_image(self, evidence: Dict) -> Optional[str]:
        ...
        logger.info(f"Attempting to generate image for evidence {evidence_id} using {self.provider}")
        ...
        if original_ext not in ['.jpg', '.jpeg', '.png']:
            logger.debug(f"Unsupported extension '{original_ext}' for {evidence_id}, defaulting to PNG")
            original_ext = '.png'
            
        logger.debug(f"Will save image as {original_ext} format for {evidence_id}")

        if self.dry_run:
            logger.info(f"[DRY RUN] Would generate image for {evidence_id} with prompt: '{prompt}'")
        ...
        for attempt in range(self.max_retries):
            try:
                logger.info(f"Attempt {attempt + 1}/{self.max_retries} for evidence {evidence_id}")
                ...
                if image_data:
                    ...
                    if is_valid:
                        ...
                        try:
                            ...
                            if save_format == 'JPEG':
                                ...
                                if img.mode == 'RGBA':
                                    logger.debug(f"Converting RGBA to RGB for JPEG output for {evidence_id}")
                            ...
                            logger.info(f"Image successfully generated and saved to {filepath} as {save_format}")
                            
                            # Save to image_assets table using local_file as the path key
                            db_path = original_path.lstrip('/')
                            self._save_to_image_assets(db_path, processed_image_data, image_url)
                            ...
                            with open(filepath, 'wb') as f:
                                f.write(image_data)
                            logger.info(f"Image saved in original format to {filepath} (format conversion failed)")
                        ...
                        if image_url:
                            ...
                            if current_data_row and current_data_row[0]:
                                try:
                                    ...
                                    logger.warning(f"Failed to parse existing evidence data for {evidence_id}")
                    ...
                    if attempt < self.max_retries - 1:
                        ...
                        logger.warning(f"Generation attempt {attempt + 1} failed. Retrying in {wait_time}s...")
            ...
            # Catch specific provider errors if available/needed
                 ...
                 wait_time = self.rate_limit_delay * (2 ** attempt)
                 logger.warning(f"OpenAI/Azure Rate limit hit, waiting {wait_time}s. Error: {e}")
                 time.sleep(wait_time)
                 error_message = str(e)
            ...
            # Together AI errors
                 ...
                 wait_time = self.rate_limit_delay * (2 ** attempt)
                 logger.warning(f"Together AI Rate limit hit, waiting {wait_time}s. Error: {e}")
                 time.sleep(wait_time)
                 error_message = str(e)
                 ...
                 if "model_not_available" in str(e):
                     logger.error(f"Model {self.image_model_name} is not available in Together AI")
                     return None
            ...
            # Add specific Google/Together errors here if known
                ...
                logger.exception(f"Unexpected error generating image for {evidence_id} (attempt {attempt + 1}): {e}")
                error_message = str(e)
                if attempt < self.max_retries - 1:
                    wait_time = self.rate_limit_delay * (2 ** attempt)
        ...
        logger.error(f"Failed to generate image for {evidence_id} after {self.max_retries} attempts. Last error context: {error_message}")
        return None
    ...
    def _generate_image_openai(self, prompt: str) -> Optional[bytes]:
        ...
        try:
            logger.debug(f"Calling OpenAI API model={self.image_model_name}, size={self.image_size}, quality={self.image_quality}")
            ...
            logger.debug(f"Image downloaded from OpenAI, size: {len(image_response.content)} bytes")
            return image_response.content
            
        except (openai.BadRequestError, openai.RateLimitError) as e: raise e 
        except requests.exceptions.RequestException as e:
    ...
    def _generate_image_azure(self, prompt: str) -> Optional[bytes]:
        ...
        try:
            logger.debug(f"Calling Azure OpenAI API deployment={self.image_model_name}, size={self.image_size}, quality={self.image_quality}")
            ...
            image_response = requests.get(image_url, timeout=60) 
            image_response.raise_for_status()
            logger.debug(f"Image downloaded from Azure, size: {len(image_response.content)} bytes")
            return image_response.content
    ...
    def _log_together_response(self, response, level=logging.DEBUG, prefix="Together API response"):
        ...
        try:
            ...
            if hasattr(response, 'data') and response.data:
                for i, item in enumerate(response.data):
                    logger.log(level, f"{prefix} data[{i}] properties available: {dir(item)}")
                    # Log interesting fields in the data items
                    for item_field in ['revised_prompt', 'url', 'b64_json', 'finish_reason']:
                        if hasattr(item, item_field):
                            value = getattr(item, item_field)
                            # Truncate long values
                            if isinstance(value, str) and len(value) > 100:
                                value = value[:100] + "..."
                            logger.log(level, f"{prefix} data[{i}].{item_field}: {value}")
        except Exception as e:
            logger.warning(f"Error logging Together API response: {e}")

    def _generate_image_together(self, prompt: str) -> Union[Optional[bytes], Tuple[bytes, str]]:
        """
        ...
        """
        try:
            logger.debug(f"Calling Together AI API model={self.image_model_name}, height={self.height}, width={self.width}")
            ...
            self._log_together_response(response, logging.DEBUG, "Together API success response")
            
            # Check if image data is present in response
            if not response.data:
                logger.warning("Together AI API returned no data items.")
                self._log_together_response(response, logging.WARNING, "Together API empty response")
                return None
            ...
            # No usable image data found
                ...
                logger.warning("Together AI API response contained no usable image data (neither b64_json nor url).")
                self._log_together_response(response, logging.WARNING, "Together API response without image data")
                return None
                
            # Validate and return the image data
            if image_data:
                logger.debug(f"Image generated by Together AI, size: {len(image_data)} bytes")
                return image_data
            else:
                return None
             ...
             if hasattr(e, 'response') and e.response:
                 logger.error(f"Rate limit details: Status {e.response.status_code}, {e.response.text}")
                 try:
                     ...
                     logger.debug(f"Could not parse rate limit error response as JSON: {json_err}")
             ...
             # Log detailed error info if available
             if hasattr(e, 'response') and e.response:
                 logger.error(f"Auth error details: Status {e.response.status_code}, {e.response.text}")
                 try:
                     # Try to parse response JSON
                     if hasattr(e.response, 'json'):
                         error_data = e.response.json()
                     ...
                     logger.debug(f"Could not parse auth error response as JSON: {json_err}")
             ...
             # Log detailed error info if available
             if hasattr(e, 'response') and e.response:
                 logger.error(f"Error response details: Status {e.response.status_code}, {e.response.text}")
                 try:
                     # Try to parse response JSON
                     if hasattr(e.response, 'json'):
                         error_data = e.response.json()
                     ...
                     logger.debug(f"Could not parse API error response as JSON: {json_err}")
             ...
             if hasattr(e, 'response') and e.response:
                 logger.error(f"Request error details: Status {e.response.status_code}, {e.response.text}")
                 try:
                     # Try to parse response JSON
                     if hasattr(e.response, 'json'):
                         error_data = e.response.json()
                     ...
                     logger.debug(f"Could not parse request error response as JSON: {json_err}")
    ...
    def generate_missing_images(self, limit: Optional[int] = None) -> Dict:
        ...
        if not missing_evidence: return results
        logger.info(f"Starting image generation for {len(missing_evidence)} records using {self.provider}...")
        ...
        for evidence in tqdm(missing_evidence, desc="Generating images"):
            ...
            try:
                ...
                generated_path = self.generate_image(evidence)
                 ...
                 results["details"].append({"evidence_id": evidence["evidence_id"], "status": "interrupted", "error": "User interrupted", "prompt": self.generate_prompt(evidence), "duration_s": round(time.time() - start_time, 2)})
                 break 
            except Exception as e:
                logger.exception(f"Critical error during generation loop for {evidence['evidence_id']}: {e}")
                error_info = f"Loop error: {e}"

            elapsed_time = time.time() - start_time
            if generated_path:
                results["successful"] += 1
                results["details"].append({"evidence_id": evidence["evidence_id"], "status": "success", "path": generated_path, "prompt": self.generate_prompt(evidence), "duration_s": round(elapsed_time, 2)})
            else:
                results["failed"] += 1
                results["details"].append({"evidence_id": evidence["evidence_id"], "status": "failed", "error": f"Image generation failed after retries (check logs). Last error context: {error_info}", "prompt": self.generate_prompt(evidence), "duration_s": round(elapsed_time, 2)})
    ...
    def test_prompt_generation(self, evidence_id: str) -> None:
        ...
        evidence = {
            k: v for k, v in zip(
                ["evidence_id", "thing_id", "account_id", "evidence_type", "thing_title", 
                 "thing_description", "category", "brand", "model", "color", "evidence_data", "local_file"], 
                row
            )
        }
        ...
        # Parse evidence data
        try:
            evidence["evidence_data"] = json.loads(row[10]) if row[10] and row[10] != 'null' else {}
        except json.JSONDecodeError:
            logger.warning(f"Failed to parse evidence data for ID {evidence_id}")
        ...
        if not file_path.exists():
            ...
            # 1. Check for URL in evidence data
            image_url = evidence["evidence_data"].get('image_url') or evidence["evidence_data"].get('url')
            ...
            # 3. If we found a URL, try to download the image
            if image_url:
                logger.info(f"Found URL for missing test image {evidence_id}: {image_url[:100]}...")
                if self._recover_image_from_url(image_url, file_path):
        ...
        # Default to PNG if no extension or unsupported extension
        if original_ext not in ['.jpg', '.jpeg', '.png']:
            logger.debug(f"Unsupported extension '{original_ext}' for test prompt, defaulting to PNG")
            original_ext = '.png'
        ...
        logger.info(f"Testing prompt with {self.provider} provider, will save as {save_format}...")
        ...
        try:
            if self.provider == "google":
                response = self.google_model.generate_images(prompt=prompt, number_of_images=1)
                logger.info(f"Prompt accepted by Google Vertex AI. Response snippet: {str(response)[:200]}...")
            elif self.provider == "openai":
                response = self.openai_client.images.generate(
                    model=self.image_model_name, prompt=prompt, size=self.image_size, quality=self.image_quality, n=1
                )
                logger.info(f"Prompt accepted by OpenAI DALL-E API. Response snippet: {str(response)[:200]}...")
            elif self.provider == "azure":
                response = self.azure_client.images.generate(
                    model=self.image_model_name, prompt=prompt, size=self.image_size, quality=self.image_quality, n=1
                )
                logger.info(f"Prompt accepted by Azure OpenAI API. Response snippet: {str(response)[:200]}...")
            elif self.provider == "together":
                ...
                self._log_together_response(response, logging.INFO, "Together test prompt response")
                    ...
                    if hasattr(data_item, 'b64_json') and data_item.b64_json:
                        ...
                        if not self.dry_run:
                            try:
                                ...
                                img.save(test_filepath, format=save_format, **save_options)
                                logger.info(f"Test image saved to {test_filepath} as {save_format}")
                            except Exception as img_err:
                                logger.error(f"Failed to save test image: {img_err}")
                        
                    elif hasattr(data_item, 'url') and data_item.url:
                        logger.info(f"Together API returned an image URL: {data_item.url[:100]}...")
                        # Optionally save the test image
                        if not self.dry_run:
                            test_filename = f"test_prompt_{evidence_id}_{datetime.now():%Y%m%d_%H%M%S}{original_ext}"
                            test_filepath = self.output_dir / test_filename
                            if self._recover_image_from_url(data_item.url, test_filepath):
            ...
            if self.provider == "together":
                if hasattr(e, 'response') and e.response:
                    logger.error(f"Together API error response: Status {e.response.status_code}, {e.response.text}")
                    try:
                        if hasattr(e.response, 'json'):
                            error_data = e.response.json()
                            logger.error(f"Together API error details: {error_data}")
                    except Exception as json_err:
                        logger.debug(f"Could not parse Together API error response as JSON: {json_err}")

    def _log_prompt_analytics(self, prompt: str, evidence_id: str, is_document: bool = None) -> None:
        """
        ...
        try:
            ...
            if is_document is None:
                is_document = (
                    "document type:" in prompt or 
                    any(term in prompt.lower() for term in ["receipt", "invoice", "certificate", "letter", "id", "identification"])
                )
    ...
    def _recover_image_from_url(self, url: str, file_path: Path) -> bool:
        ...
        try:
            ...
            path = str(file_path.relative_to(self.output_dir)) if self.output_dir in file_path.parents else str(file_path)
...
def main():
    ...
    try:
        ...
        if not project_id: print("Skipping Google test: GOOGLE_CLOUD_PROJECT env var not set.")
        else:
            ...
            with open(results_file_google, 'w') as f: json.dump(results_google, f, indent=2)
            logger.info(f"Google results saved to {results_file_google}")
    except Exception as e: logger.error(f"Error in main (Google): {str(e)}")

    # OpenAI
    try:
         if not os.getenv("OPENAI_API_KEY"): print("Skipping OpenAI test: OPENAI_API_KEY env var not set.")
         else:
            ...
            with open(results_file_openai, 'w') as f: json.dump(results_openai, f, indent=2)
            logger.info(f"OpenAI results saved to {results_file_openai}")
    except Exception as e: logger.error(f"Error in main (OpenAI): {str(e)}")

    # Azure OpenAI
    try:
        if not all(os.getenv(k) for k in ["AZURE_OPENAI_API_KEY", "AZURE_OPENAI_ENDPOINT", "AZURE_OPENAI_DEPLOYMENT_NAME", "AZURE_OPENAI_API_VERSION"]):
            print("Skipping Azure test: Required Azure environment variables not set.")
        else:
            logger.info("\n--- Testing Azure Provider ---")
            generator_azure = EvidenceImageGenerator(
                provider="azure"
            )
            ...
            with open(results_file_azure, 'w') as f: json.dump(results_azure, f, indent=2)
            logger.info(f"Azure results saved to {results_file_azure}")
    except Exception as e: logger.error(f"Error in main (Azure): {str(e)}")
    ...
    try:
        if not os.getenv("TOGETHER_API_KEY"):
            ...
            with open(results_file_together, 'w') as f: json.dump(results_together, f, indent=2)
