...
class PreviewImageGenerator(EvidenceImageGenerator):
    ...
    def generate_image(self, evidence: Dict) -> Optional[str]:
        ...
        if original_ext not in [".jpg", ".jpeg", ".png"]:
            logger.debug(
                f"Unsupported extension '{original_ext}' for preview {evidence_id}, defaulting to PNG"
            )
            original_ext = ".png"

        logger.debug(
            f"Will save preview image as {original_ext} format for {evidence_id}"
        )
        ...
        for attempt in range(self.max_retries):
            try:
                logger.info(
                    f"Attempt {attempt + 1}/{self.max_retries} for preview {evidence_id}"
                )
                ...
                if self.provider == "openai":
                    ...
                    if isinstance(result, tuple) and len(result) == 2:
                        image_data, _ = (
                            result  # Discard the URL since we don't store it for previews
                        )
                    else:
                        image_data = result
                else:
                ...
                if image_data:
                    ...
                    if is_valid:
                        ...
                        try:
                            ...
                            if save_format == "JPEG":
                                ...
                                if img.mode == "RGBA":
                                    logger.debug(
                                        f"Converting RGBA to RGB for JPEG preview for {evidence_id}"
                                    )
                            ...
                            logger.info(
                                f"Preview image successfully saved to {filepath} as {save_format}"
                            )
                            ...
                            with open(filepath, "wb") as f:
                                f.write(image_data)
                            logger.info(
                                f"Preview image saved in original format to {filepath} (format conversion failed)"
                            )
                    ...
                    if attempt < self.max_retries - 1:
                        ...
                        logger.warning(
                            f"Preview generation attempt {attempt + 1} failed. Retrying in {wait_time}s..."
                        )
                ...
                logger.warning(
                    f"Preview generation for {evidence_id} interrupted by user."
                )
                return None
            except Exception as e:
                ...
                if attempt < self.max_retries - 1:
                    wait_time = self.rate_limit_delay * (2**attempt)
                    logger.warning(
                        f"Preview generation attempt {attempt + 1} failed with error: {error_message}. Retrying in {wait_time}s..."
                    )
                    time.sleep(wait_time)
                else:
                    logger.error(
                        f"All preview generation attempts failed for {evidence_id}: {error_message}"
                    )
                    return None
...
def display_generated_images(results: Dict):
    ...
    successful_images = [
        ...
        if d.get("status") == "success" and "path" in d
    ]
...
def generate_preview_images(
    ...
    try:
        ...
        for evidence in tqdm(missing_evidence, desc=f"Generating {provider} previews"):
            ...
            try:
                time.sleep(
                    generator.rate_limit_delay if results["attempted"] > 1 else 0
                )
                ...
                results["details"].append(
                    {
                        ...
                        "prompt": generator.generate_prompt(evidence),
                ...
                logger.exception(
                    f"Critical error during preview loop for {evidence['evidence_id']}: {e}"
                )
                ...
                results["details"].append(
                    {
                        "evidence_id": evidence["evidence_id"],
                        "status": "failed",
                        "error": f"Preview generation failed (check logs). Last error context: {error_info}",
                        "prompt": generator.generate_prompt(evidence),
                        "duration_s": round(elapsed_time, 2),
                    }
                )
        ...
        if results["details"]:
            ...
            display(
                df[
                    ["evidence_id", "status", "prompt", "error", "duration_s", "path"]
                ].fillna("N/A")
...
if __name__ == "__main__":
    ...
    print(
        f"Preview generation completed with {results['successful']} successful generations."
    )
