...
def test_load_taxonomy_hierarchy():
    ...
    pets = next(
        c for c in hierarchy["children"] if c["name"] == "Animals & Pet Supplies"
    )
...
def test_lookup_code_by_name():
    ...
    code = lookupCodeByName(
        "Apparel & Accessories > Clothing > Activewear", SAMPLE_TAXONOMY
    )
    assert code == "3003"
...
def test_lookup_all_codes():
    root = {
        "name": "root",
        "code": None,
        "children": [{"name": "Animals & Pet Supplies", "code": None, "children": []}],
    }
    with patch("requests.get") as mock_get:
        mock_get.return_value.text = SAMPLE_TAXONOMY
