...
ProductType = namedtuple(
    "ProductType", BaseType._fields + ("gpcRoot", "gpcName", "gpcCode", "unspscID")
)
DocumentType = namedtuple("DocumentType", BaseType._fields)
EvidenceType = namedtuple(
    "EvidenceType", BaseType._fields + ("gpcRoot", "gpcName", "gpcCode", "unspscID")
)
...
def generate_evidence_types() -> Generator[EvidenceType, None, None]:
    ...
    for provider, doc_types in DOCUMENT_PROVIDERS:
        for doc_type in doc_types:
            ...
            yield EvidenceType(
                ...
                gpcName=doc_type,
                gpcCode=80000
                + hash(f"{provider}_{doc_type}") % 1000,  # Generate stable code
                unspscID=None,
            )
    ...
    categories = taxonomy_manager.categories_list[
        :100
    ]  # Limit to first 100 for practical purposes
