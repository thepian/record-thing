...
class B2Sync:
    ...
    def _sync(self):
        for root, dirs, files in os.walk(self.local_folder):
            for file in files:
                ...
                s3_path = relative_path.replace("\\", "/")  # Ensure compatibility with S3 path format
    ...
    def sync(self, local_path: str, max_workers: int = 4) -> None:
        ...
        for relative_path, local_hash in local_files.items():
            if relative_path not in remote_files or remote_files[relative_path] != local_hash:
...
if __name__ == "__main__":
    ...
    b2sync.sync()
    class B2Config:
        def __init__(self, local_folder, bucket_name, aws_access_key_id, aws_secret_access_key, region_name='us-west-002'):
