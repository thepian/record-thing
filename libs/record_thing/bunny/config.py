...
class BunnyConfig:
    ...
    def __post_init__(self):
        ...
        if not self.api_key or len(self.api_key) < 32:
            raise ValueError("Invalid API key format. Bunny.net API keys should be at least 32 characters long.")
                
        # Validate storage zone name format
        if not re.match(r'^[a-z0-9][a-z0-9-]{2,62}$', self.storage_zone_name.lower()):
            raise ValueError("Invalid storage zone name. Names must be 3-63 characters and contain only lowercase letters, numbers, and hyphens.")
            
        # Validate hostname format
        if not re.match(r'^([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)+[a-z0-9]([a-z0-9-]*[a-z0-9])?$', self.hostname.lower()):
            raise ValueError("Invalid hostname format. Should be a valid domain name (e.g., storage.bunnycdn.com).")
