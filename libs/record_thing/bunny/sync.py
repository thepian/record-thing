...
class BunnySync:
    ...
    def __init__(self, config: BunnyConfig):
        ...
        self.session.headers.update(
            {"AccessKey": config.api_key, "Accept": "application/json"}
        )
        ...
        if config.use_bunnycdn_package:
            if not BUNNY_PACKAGE_AVAILABLE:
                logger.warning(
                    "bunnycdnpython package not installed. Falling back to direct API calls."
                )
            elif not config.storage_api_key:
                logger.warning(
                    "storage_api_key not provided. Falling back to direct API calls."
                )
            else:
                try:
                    ...
                    storage_region = (
                        config.storage_region if config.storage_region else ""
                    )
                    self.bunny_client = Storage(
                        config.storage_api_key, config.storage_zone_name, storage_region
                    )
                    logger.info("Using bunnycdnpython package for storage operations")
                except Exception as e:
                    logger.error(
                        f"Failed to initialize bunnycdnpython client: {str(e)}"
                    )
                    self.bunny_client = None
    ...
    def _test_api_connection(self) -> None:
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                raise ValueError(
                    f"Storage zone '{self.config.storage_zone_name}' not found"
                )
    ...
    def _test_password_connection(self) -> None:
        ...
        try:
            ...
            if response.status_code == 401:
                raise ValueError(
                    "Password authentication failed: Invalid password or storage zone name"
                )
            elif response.status_code == 404:
                # 404 might be acceptable if the root directory is empty
                logger.info(
                    "Successfully authenticated with password (root directory empty)"
                )
                return
            elif response.status_code != 200:
                raise ValueError(
                    f"Failed to connect with password: {response.status_code} {response.text}"
                )

            logger.info(
                "Successfully connected to Bunny.net using password authentication"
            )
    ...
    def _get_remote_files(self, path: Path | str = None) -> Dict[str, Dict[str, Any]]:
        ...
        if self.bunny_client:
            try:
                ...
                try:
                        ...
                        if isinstance(item, str):
                            ...
                            if object_name:
                                if is_directory:
                                    ...
                                    subdirectory_files = self._get_remote_files(
                                        subdir_path
                                    )
                                    remote_files.update(subdirectory_files)
                                else:
                    ...
                    logger.error(
                        f"bunnycdnpython GetStoragedObjectsList error with path '{path}': {str(e)}"
                    )
        ...
        url = (
            f"{self.config.storage_api_url}/{self.config.storage_zone_name}/{url_path}"
        )
    ...
    def _download_file(
        self, remote_path: str, local_file_path: str, use_password: bool = False
    ) -> bool:
        """
    ...
    def sync(
            ...
            for root, _, files in os.walk(cache_dir):
                for file in files:
                    ...
                    if protected_patterns and any(
                        str(target_path).startswith(p) for p in protected_patterns
                    ):
                        logger.debug(
                            f"Skipping write-protected cached file: {relative_path}"
                        )
                        results["protected_skipped"] += 1
                        continue

                    if not target_path.exists() or self._calculate_checksum(
                        str(cache_file_path)
                    ) != self._calculate_checksum(str(target_path)):
        ...
        if sync_paths:
            ...
            for sync_path in sync_paths:
                ...
                if sync_path_full.is_file():
                    ...
                    relative_path = relative_path.replace("\\", "/")
                    local_files[relative_path] = self._calculate_checksum(
                        str(sync_path_full)
                    )
        ...
        to_download = []
        if sync_paths and not all(
            p.startswith("!") for p in sync_paths
        ):  # Skip if all paths are exclude patterns
        ...
        if dry_run:
            ...
            logger.info(
                f"DRY RUN: Would skip {results['protected_skipped']} write-protected files"
            )
            return results
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for i, relative_path in enumerate(to_upload):
                    ...
                    task = executor.submit(
                        self._upload_file, local_file_path, relative_path
                    )
                    upload_tasks.append((task, relative_path))

                    if progress_callback:
                        progress_callback(
                            i + 1, len(to_upload), "uploading", relative_path
                        )
        ...
        if to_delete:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for i, remote_path in enumerate(to_delete):
                    ...
                    if progress_callback:
                        progress_callback(
                            i + 1, len(to_delete), "deleting", remote_path
                        )
                ...
                for task, remote_path in delete_tasks:
                    try:
                        ...
                        results["failed"] += 1
                        results["errors"].append(
                            f"Error deleting {remote_path}: {str(e)}"
                        )
        ...
        logger.info(
            f"Sync completed: {results['uploaded']} uploaded, {results['downloaded']} downloaded, {results['deleted']} deleted, {results['unchanged']} unchanged, {results['cached']} cached from local cache, {results['protected_skipped']} write-protected skipped, {results['failed']} failed"
        )
        return results
    ...
    def purge_cache(self, paths: List[str] = None) -> Dict[str, Any]:
        ...
        results = {"success": False, "purged": 0, "failed": 0, "errors": []}
...
if __name__ == "__main__":
    ...
    bunny_sync._download_file(
        "path/to/file.jpg", "./assets/cache/downloaded_file.jpg", use_password=True
    )
