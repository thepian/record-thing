...
struct ExampleCardViewData: CardViewData {
    ...
    init(id: UUID = UUID(), index: Int, title: String, color: Color = .red, imageName: String? = nil) { // "thepia_a_high-end_electric_mountain_bike_1"
        self.id = id
        self._index = index
        self.title = title
        if let imageName = imageName {
            self.image = Image(imageName, bundle: Bundle.module)
        } else {
            self.image = nil
        }
        self.color = color
...
extension Carousel {
    struct CardView: View {
        ...
        var body: some View {
            let cardWidth = designSystem.evidenceReviewWidth
            let cardHeight = designSystem.evidenceReviewHeight
