...
public struct GridPopupCard<RowType: Sendable, Content: View>: View
where RowType: Equatable, RowType: Identifiable {
  ...
  public init(
    ...
    @ViewBuilder itemContent: @escaping (RowType, Bool, @escaping () -> Void, @escaping () -> Void)
      -> Content
  ) {
    self.item = item
...
public struct GridWithPopup<
  RowType: Sendable, HeaderView: View, BottomBar: View, ItemContent: View
>: View where RowType: Equatable, RowType: Identifiable {
  ...
  public var body: some View {
    ZStack {
      if isScrollable {
        ...
        // Use VStack for fixed-height groups (asset browsing)
        VStack(spacing: 0) {
          content
            #if os(macOS)
              .frame(maxWidth: 600)
