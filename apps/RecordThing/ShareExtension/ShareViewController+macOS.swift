...
class ShareViewController: NSViewController {
  ...
  override func viewDidLoad() {
    ...
    print("📱 SHAREEXTENSION (macOS): Extension bundle ID: \(Bundle.main.bundleIdentifier ?? "unknown")")
  ...
  private func extractSharedContentForDisplay() async -> SharedContent? {
    ...
    for attachment in attachments {
      if attachment.hasItemConformingToTypeIdentifier("public.plain-text") {
        do {
          let textData = try await attachment.loadItem(forTypeIdentifier: "public.plain-text", options: nil)
  ...
  private func logExtensionContext() {
    ...
    logger.info("📋 Extension context available with \(extensionContext.inputItems.count) input items")
  ...
  private func extractSharedContent() {
    Task {
      if let content = await extractSharedContentForDisplay() {
        logger.info("📥 Extracted shared content: \(content.title)")
  ...
  private func processSharedContent(with note: String) async {
    ...
    if let content = await extractSharedContentForDisplay() {
      logger.info("💾 Would save content: \(content.title)")
      if let url = content.url {
        logger.info("🔗 URL: \(url.absoluteString)")
      }
