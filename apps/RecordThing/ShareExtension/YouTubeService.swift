...
class YouTubeService {
    private let logger = Logger(subsystem: "com.thepia.recordthing.shareextension", category: "YouTubeService")
    ...
    func fetchThumbnail(for videoId: String, quality: ThumbnailQuality = .high) async -> UIImage? {
        ...
        for thumbnailQuality in qualities {
            ...
            do {
                ...
                logger.warning("Failed to fetch thumbnail at \(thumbnailQuality.rawValue) quality: \(error.localizedDescription)")
                continue
            }
        }
