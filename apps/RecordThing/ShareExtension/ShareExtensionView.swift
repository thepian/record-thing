...
struct ShareExtensionView: View {
  ...
  // Environment access to datasource
  @Environment(\.shareExtensionDatasource) private var datasource
                ...
                if geometry.size.width > 600 {
                  ...
                  HStack(alignment: .bottom, spacing: 12) {
                    TextEditorWithButton(
                      ...
                      onSend: {
                        logger.info(
                          "User initiated send action (iPad layout) with note length: \(note.count)"
                        )
                        isLoading = true
                        onSave(note)
                      }
                  ...
                  TextEditorWithButton(
                    ...
                    onSend: {
                      logger.info(
                        "User initiated send action (iPhone layout) with note length: \(note.count)"
                      )
                      isLoading = true
                      onSave(note)
                    }
    ...
    VStack(spacing: 0) {
      ...
      if let heroImage = content.heroImage {
        heroImage.asImage
          ...
          .frame(height: 120)
          .clipped()
          .cornerRadius(8, recordCorners: [.topLeft, .topRight])
      ...
      .cornerRadius(8, recordCorners: [.bottomLeft, .bottomRight])
    }
    .overlay(
      RoundedRectangle(cornerRadius: 8)
        .stroke(Color.secondarySystemBackground, lineWidth: 1)
