        ...
        ScrollView {
            VStack(alignment: .leading) {
                ...
                VStack(alignment: .leading) {
                    ...
                    VStack {
                        ForEach(0 ..< product.measuredIngredients.count, id: \.self) { index in
                            RecipeIngredientRow(measuredIngredient: product.measuredIngredients[index].scaled(by: Double(smoothieCount)))
                                .padding(.horizontal)
                            if index < product.measuredIngredients.count - 1 {
                                Divider()
                            }
                        }
                    }
...
struct RecipeView_Previews: PreviewProvider {
    static var previews: some View {
        RecipeView(product: .thatsBerryBananas)
