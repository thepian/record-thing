            ...
            if !monitor.activities.isEmpty {
                VStack(alignment: .leading, spacing: 12) {
                    HStack {
                        ...
                        Text("Recent Database Activities")
                            .font(.headline)
                    }
                    
                    VStack(spacing: 4) {
                        ForEach(Array(monitor.activities.prefix(3))) { activity in
                            HStack(spacing: 8) {
...
struct DatabaseErrorView_Previews: PreviewProvider {
    static var previews: some View {
        Group {
            // Blackbird Error 7 (NOMEM)
            DatabaseErrorView(
                error: NSError(domain: "Blackbird.Database.Error", code: 7, userInfo: [
                    NSLocalizedDescriptionKey: "Blackbird.Database.Error error 7"
                ]),
                context: "Failed to load assets from database",
                onRetry: { print("Retry tapped") },
                onShowDashboard: { print("Dashboard tapped") }
            )
            .environmentObject(AppDatasource.shared)
            .previewDisplayName("Blackbird Error 7")
            
            // Generic Database Error
            DatabaseErrorView(
                error: NSError(domain: "Database", code: 1, userInfo: [
