...
extension Bundle {
    ...
    static func installBlackbirdTranslations() {
        ...
        if let methodOriginal = class_getClassMethod(bundleClass, #selector(localizedString(forKey:value:table:))),
           let methodCustom = class_getClassMethod(bundleClass, #selector(blackbirdLocalizedString(forKey:value:table:))) {
            method_exchangeImplementations(methodOriginal, methodCustom)
        }
        logger.trace("installed Translations in Bundle")
    }
    
    @objc private class func blackbirdLocalizedString(forKey key: String, value: String?, table: String?) -> String {
...
//    }
    ...
    func loadTranslations(for locale: String?) async {
        ...
        do {
            ...
            let rows: [Dictionary] = try await db
                .query("""
            ...
            let keysWithValues = rows.compactMap { ($0["key"]?.stringValue ?? "", $0["value"]?.stringValue ?? "") }
