...
struct ShareExtensionPerformanceTests {
  ...
  @Test func testYouTubeURLRecognitionPerformance() async throws {
    ...
    #expect(
      timeElapsed < 1.0,
      "YouTube URL recognition should be fast: \(timeElapsed)s for 8000 operations")
  }
  ...
  @Test func testVideoIDExtractionPerformance() async throws {
    ...
    // Should complete 1000 iterations of 8 URLs (8000 operations) in under 1 second
    #expect(
      timeElapsed < 1.0, "Video ID extraction should be fast: \(timeElapsed)s for 8000 operations")
  }

  @Test func testThumbnailURLGenerationPerformance() async throws {
    let videoIds = TestConfiguration.sampleVideoIds
    let qualities: [YouTubeService.ThumbnailQuality] = [
    ...
    for _ in 0..<1000 {
      for videoId in videoIds {
        for quality in qualities {
          _ = YouTubeService.getThumbnailURL(for: videoId, quality: quality)
        }
      }
    }
    ...
    #expect(
      timeElapsed < 1.0,
      "Thumbnail URL generation should be fast: \(timeElapsed)s for 25000 operations")
  }
  ...
  @Test func testSharedContentCreationPerformance() async throws {
    ...
    #expect(
      timeElapsed < 0.1,
      "SharedContent creation should be fast: \(timeElapsed)s for 1000 operations")
  }
  ...
  @Test func testDisplayTitleGenerationPerformance() async throws {
    ...
    #expect(
      timeElapsed < 0.05,
      "Display title generation should be fast: \(timeElapsed)s for 1000 operations")
  }
  ...
  @Test func testViewModelUpdatePerformance() async throws {
    ...
    #expect(
      timeElapsed < 0.01, "ViewModel updates should be fast: \(timeElapsed)s for 100 operations")
  }
  ...
  @Test func testViewModelStateChangesPerformance() async throws {
    ...
    #expect(
      timeElapsed < 0.1,
      "ViewModel state changes should be fast: \(timeElapsed)s for 1000 operations")
  }
